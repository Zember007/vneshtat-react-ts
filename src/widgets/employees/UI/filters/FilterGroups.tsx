...
const FilterGroups = ({ selectedGroupId }: { selectedGroupId: number | null }) => {
    ...
    const Group = useSelector((state: RootState) => state.employees.GroupsInformation).find(item => item.id === selectedGroupId);
                            ...
                            <>
                                <div className="flex flex-col gap-[10px]">
                                    ...
                                    <div className="flex flex-col gap-[6px]">
                                        {
                                            Group?.Passengers && Group?.Passengers.map(item => (
                                                <CartEmployee name={item.Name} surname={item.Surname} middlename={item.MiddleName} id={item.id} departamentId={Group.id} />
                                            ))
                                        }
                                        {employeeAdd &&
                                            <CartEmployee add={() => { setEmployeeAdd(false) }} departamentId={Group?.id} />
                                        }
                                    </div>
                                </div>
...
const CartEmployee = (props: props) => {
    ...
    const DelEmployee = async (id: number) => {
        ...
        try {
            const res = await fetch(import.meta.env.VITE_API_URL + '/company/employees_profile/delete_passenger_from_group', {
                method: "DELETE",
                headers: {
                    Authorization: `Bearer ${AccessToken}`
                },
            ...
            if (data.status === "success") {
                console.log(data);
                dispatch(delGroupEmployee({ id: props.departamentId, id_employee: id }))
            }
