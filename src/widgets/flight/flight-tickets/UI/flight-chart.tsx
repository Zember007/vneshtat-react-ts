import { Bar, BarChart, Brush, Cell, Line, LineChart, ReferenceLine, ResponsiveContainer, Tooltip, XAxis } from "recharts";
...
const FlightChart = ({ showedGraph, setShowedGraph, activeRate }: {
    ...
    const checkDatesRate = (month: string, day: number) => {
        if (flights[0] && flights[1]) {

            const firstDate = flights[0].flightDate
            const secondDate = flights[1].flightDate

            if (firstDate && secondDate) {

                const NumberMonth = months.findIndex(item => item.month === month)
                
                
                
                if (firstDate.getMonth() <= NumberMonth && NumberMonth <= secondDate.getMonth() && firstDate.getDate() <= day && day <= secondDate.getDate()) {
                    return true
                }
            }
        }
                        ...
                        <ResponsiveContainer width="100%" height={185} style={{ position: "relative", zIndex: "30" }}>
                            <BarChart
                                ...
                                <Bar dataKey="uv">
                                    {
                                        data.map((item, index) => {
                                            const color =  checkDatesRate(item.name, item.day) ? '#ff866e' : '#BFBEFC'
                                            return (
